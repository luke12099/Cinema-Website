...
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath()%>/front_end/tk_ord/styles/chooseSeat.css">
<script
...
// 	長座位==================================================
	let body = document.getElementsByClassName('seatsChart')[0];
...
//  取出之前選的票數=====================================================
	let TKorderStr = sessionStorage.getItem('TKorder');
	let TKorder = JSON.parse(TKorderStr);
    let TotalCount = 0;
	...
	$(".seatNumber").click(function() {
		...
		if (!$(this).hasClass("seatUnavailable") && !$(this).hasClass("seatConstruction") && !$(this).hasClass("seatReserved")) {
			...
			if ($(this).hasClass("seatSelected")) {
                ...
                //更新資料庫座位
				let url = "${pageContext.request.contextPath}/front/tkOrd.do?action=updateShowingByShowingID&SH_ID=" + showingID + "&SH_SEAT_STATE=" + seat;
				$.ajax({
					url: url,
            		type: 'post',
           			dataType: 'json',
